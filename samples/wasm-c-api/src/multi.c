...
// A function closure.
own wasm_trap_t *
closure_callback(void *env, const wasm_val_t args[], wasm_val_t results[])
{
    int i = *(int *)env;
...
main(int argc, const char *argv[])
{
    ...
    wasm_extern_vec_new(
        &imports, 1, (wasm_extern_t *[]){ wasm_func_as_extern(callback_func) });
    ...
    wasm_val_vec_new(&args, 4,
                     (wasm_val_t[]){ WASM_I32_VAL(1), WASM_I64_VAL(2),
                                     WASM_I64_VAL(3), WASM_I32_VAL(4) });
    wasm_val_vec_new(&results, 4,
                     (wasm_val_t[]){ WASM_INIT_VAL, WASM_INIT_VAL,
                                     WASM_INIT_VAL, WASM_INIT_VAL });
    ...
    printf("> %" PRIu32 " %" PRIu64 " %" PRIu64 " %" PRIu32 "\n",
           results.data[0].of.i32, results.data[1].of.i64,
           results.data[2].of.i64, results.data[3].of.i32);
