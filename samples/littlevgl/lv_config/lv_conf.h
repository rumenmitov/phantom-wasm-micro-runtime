 ...
 * LV_DPI / 2 -> half inch wide (Not so important, you can adjust it to modify
 * default sizes and spaces)*/
#define LV_DPI 100
 ...
 * `disp_drv.disp_map` functions will be called to draw to the frame buffer
 * directly*/
...
/* Using true double buffering in `disp_drv.disp_flush` you will always get the
 * image of the whole screen. Your only task is to set the rendered image
 * (`color_p` parameter) as frame buffer address or send it to your display. The
 * best if you do in the blank period of you display to avoid tearing effect.
 * Requires:
...
#define USE_LV_FILESYSTEM \
    0 /*1: Enable file system (might be required for images*/
...
#define LV_TICK_CUSTOM                                                        \
    1 /*1: use a custom tick source (removing the need to manually update the \
         tick with `lv_tick_inc`) */
#if LV_TICK_CUSTOM == 1
#define LV_TICK_CUSTOM_INCLUDE \
    "system_header.h" /*Header for the sys time function*/
#define LV_TICK_CUSTOM_SYS_TIME_EXPR \
    (time_get_ms()) /*Expression evaluating to current systime in ms*/
 ...
 * LV_LOG_LEVEL_WARN        Log if something unwanted happened but didn't caused
...
#define USE_LV_THEME_NEMO \
    0 /*Water-like theme based on the movie "Finding Nemo"*/
...
#if defined(_MSC_VER) \
    && !defined(      \
        _CRT_SECURE_NO_WARNINGS) /* Disable warnings for Visual Studio*/
